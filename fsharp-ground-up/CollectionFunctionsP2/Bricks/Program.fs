...
let main argv =
    ...
    let groupedByStud =
        bricks
        |> Array.map(fun x-> {| Studs = x.StudColumns * x.StudRows; Brick = x |})
        |> Array.groupBy(fun x -> x.Studs)
        |> Array.iter(fun (studs, bricks) ->
             printfn "%i:" studs
             bricks |> Array.iter(fun y -> Brick.printConsole y.Brick)
             printfn "")
    printfn ""
    
    
    printfn "Grouped by studs (Array.groupBy) alternative:"
    let groupedByStudCount =
        bricks
        |> Array.groupBy(fun x -> x.StudColumns * x.StudRows)

    groupedByStudCount
    |> Array.sortByDescending fst
    |> Array.iter(fun (studCount, bricks) ->
             printfn "%i studs:" studCount
